...
public class Wire extends TransitiveObservable {
    ...
    public void fixWire(int v) {
        ...
        for (int i = 0; i < this.observers.size(); i++) {
            ...
            //can be understood as the circuit do not have this wire input any more
            c.inDegree--;
            //if the circuit indegree is 0, means its output will be certain independent of input
            //since there is no input, input will be fixed
            if (c.inDegree == 0) {
                c.compute();
